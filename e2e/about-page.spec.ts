...
test.describe('About Page', () => {
  ...
  test('should display mission section', async ({ page }) => {
    ...
    await expect(missionSection.getByRole('heading', { name: 'Our Mission' })).toBeVisible();
  ...
  test('should display all feature items', async ({ page }) => {
    ...
    await expect(featuresSection.getByRole('heading', { name: 'Features' })).toBeVisible();
  ...
  test('should display technology stack badges', async ({ page }) => {
    ...
    await expect(techSection.getByRole('heading', { name: 'Technology Stack' })).toBeVisible();
  ...
  test('should display team members', async ({ page }) => {
    ...
    await expect(teamSection.getByRole('heading', { name: 'Team' })).toBeVisible();

    const member1 = page.getByTestId('team-member-1');
    await expect(member1).toBeVisible();
    await expect(member1.getByRole('heading', { name: 'Raiken Bot' })).toBeVisible();
    ...
    await expect(member2.getByRole('heading', { name: 'Developer' })).toBeVisible();
  ...
  test('should have working navigation links in header', async ({ page }) => {
    ...
    await page.getByRole('link', { name: '✉️Contact' }).click();
    await expect(page).toHaveURL(`${BASE}/contact`);
  });

  test('should display brand link in header', async ({ page }) => {
    const brandLink = page.getByRole('link', { name: 'Raiken Playground' });
