...
import {
    buildCustomLoginSpec,
    formatCustomLoginFailureMessage,
    runCustomLoginScript,
} from "../browser/custom-login-runner";
...
describe("custom login runner", () => {
    ...
    it("surfaces safe failure diagnostics without child output or secrets", async () => {
        fs.mkdirSync(path.join(projectPath, "tests", "auth"), { recursive: true });
        fs.writeFileSync(
            path.join(projectPath, "tests", "auth", "login.ts"),
            "export default async () => {};",
        ...
        const previousUser = process.env["TEST_LOGIN_USER"];
        const previousPassword = process.env["TEST_LOGIN_PASSWORD"];
        process.env["TEST_LOGIN_USER"] = "leaked-user";
        process.env["TEST_LOGIN_PASSWORD"] = "leaked-password";
        ...
        spawnMock.mockImplementation((_command: string, _args: string[]) => {
            ...
            child.stderr = new PassThrough();
            queueMicrotask(() => {
                child.stderr.write("leaked-password failed auth for leaked-user");
                child.emit("close", 1);
            });
            return child;
        });
        ...
        try {
            const error = await runCustomLoginScript({ projectPath, headed: true }).catch(
                (caught) => caught,
            );
            expect(error).toBeInstanceOf(Error);
            const message = (error as Error).message;
            expect(message).toMatch(/exit 1/);
            expect(message).toContain("playwright.config.ts");
            expect(message).toMatch(/raiken-auth-.+\.spec\.ts/);
            expect(message).toContain("Headed: yes");
            expect(message).not.toContain("leaked-password");
            expect(message).not.toContain("leaked-user");
        } finally {
            if (previousUser === undefined) delete process.env["TEST_LOGIN_USER"];
            else process.env["TEST_LOGIN_USER"] = previousUser;
            if (previousPassword === undefined) delete process.env["TEST_LOGIN_PASSWORD"];
    ...
    it("formatCustomLoginFailureMessage stays generic", () => {
        const message = formatCustomLoginFailureMessage({
            exitCode: 1,
            specBasename: "raiken-auth-123.spec.ts",
            configBasename: "playwright.config.ts",
            headed: false,
        });
        expect(message).toContain("exit 1");
        expect(message).toContain("raiken-auth-123.spec.ts");
        expect(message).toContain("playwright.config.ts");
        expect(message).not.toMatch(/password|stdout|stderr/i);
    });
});
