...
export class EntryPointDetector {
    ...
    async detectEntryPoints(): Promise<EntryPointResult[]> {
        ...
        if (this.options.includeWorkspaces) {
            ...
            for (const workspaceRoot of workspaceRoots) {
                ...
                const detector = new EntryPointDetector(workspaceRoot, {
    ...
    private async detectEntryPointsForCurrentRoot(): Promise<EntryPointResult[]> {
        const [packageJsonResults, frameworkResults, conventionalResults, buildConfigResults] =
            await Promise.all([
    ...
    detectFramework(): string | null {
        ...
        else if (deps["express"] || deps["fastify"] || deps["koa"] || deps["hono"] || deps["hapi"])
            result = "node-server";
            ...
            if (typeof exports === "string") {
                ...
                if (mainExport) {
                    ...
                    if (typeof mainExport === "string") {
                        ...
                        entryPath =
                            (exportObj["import"] as string) ||
                            (exportObj["require"] as string) ||
                            (exportObj["default"] as string);
                    }
    ...
    private async detectNextJsAppRouterEntries(
        ...
        const allFiles = await fg([`${appDir}/**/*.{${extPattern}}`], {
            absolute: true,
            onlyFiles: true,
        });
        ...
        for (const file of allFiles) {
            ...
            if (basename === "layout") {
                ...
                results.push({
                    ...
                    type: "convention",
                    framework: "nextjs",
                    reason: isRoot ? "Next.js App Router root page" : "Next.js App Router page",
                    role: "page",
                });
    ...
    private async detectNextJsPagesRouterEntries(
        ...
        const specialFiles = [
            {
                name: "_app",
                reason: "Next.js Pages Router custom App component",
                role: "main" as const,
            },
            {
                name: "_document",
                reason: "Next.js Pages Router custom Document",
        ...
        if (this.fileExists(apiDir)) {
            const extPattern = pageExtensions.join(",");
            const apiFiles = await fg([`${apiDir}/**/*.{${extPattern}}`], { absolute: true });
    ...
    private detectNextJsMiddleware(): EntryPointResult[] {
        ...
        const possiblePaths = [this.projectRoot, path.join(this.projectRoot, "src")];
            ...
            const matchedPattern =
                patterns.find((p) =>
                    fg.isDynamicPattern(p)
                        ? new RegExp(p.replace(/\{.*?\}/g, ".*")).test(relativePath)
                        : relativePath === p,
    ...
    private async detectWorkspacePackageRoots(): Promise<string[]> {
        const patterns = [...this.getWorkspacePatterns(), ...this.loadPnpmWorkspacePatterns()];
