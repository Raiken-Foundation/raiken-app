...
export class CodeGraphDB {
    ...
    private migrateToV5(): void {
        const tableInfo = this.db.prepare(`PRAGMA table_info(discovery_sessions)`).all() as Array<{
            name: string;
        }>;
    ...
    private ensureFileColumns(): void {
        const tableInfo = this.db.prepare(`PRAGMA table_info(files)`).all() as Array<{
            name: string;
        }>;
        const columnNames = tableInfo.map((col) => col.name);
    ...
    private ensureDependencyColumns(): void {
        const depsInfo = this.db.prepare(`PRAGMA table_info(dependencies)`).all() as Array<{
            name: string;
        }>;
    ...
    removeFile(filePath: string): void {
        ...
        `)
                        .all(this.projectPath, filePath) as Array<{ id: number }>
                ).map((r) => r.id);
                ...
                this.db
                    ...
                    .run(this.projectPath, filePath, filePath);
                this.db
    ...
    *streamFiles(): Generator<DBFileNode> {
    ...
    clearProject(): void {
        ...
        `)
                        .all(this.projectPath) as Array<{ id: number }>
                ).map((r) => r.id);
    ...
    recordSelectorSuccess(
      ...
      `)
                .get(this.projectPath, elementDescription, selector) as
     ...
     */
    private replaceFileSymbolsInTransaction(
        filePath: string,
    ...
    getSymbolGraphStats(): { symbols: number; edges: number; edgesByKind: Record<string, number> } {
        const sym = this.db
            ...
            .get(this.projectPath) as { c: number };
        const edg = this.db
