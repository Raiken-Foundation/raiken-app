...
export async function runManualHandoff(
    ...
    const storageStatePath = options.storageStatePath ?? path.join(raikenDir, "auth-state.json");
...
function detectChange(
    ...
    const initialIsLogin =
        baseline.initialUrl === "about:blank" || looksLikeLoginUrl(baseline.initialUrl);
    if (initialIsLogin && currentUrl && !looksLikeLoginUrl(currentUrl)) return true;
    return false;
}
