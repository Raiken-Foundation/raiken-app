...
export function setCartQuantity(cart: Cart, _productId: string, _quantity: number): Cart {
    // BUG B3: quantity changes are ignored — the cart keeps the old quantity.
    return cart;
}

export function removeFromCart(cart: Cart, _productId: string): Cart {
    // BUG B7: remove is a no-op — the item stays in the cart.
    return cart;
}
...
export function validateShipping(input: ShippingInput): string | null {
    // BUG B5: ZIP validation removed — any ZIP (even "12") passes.
...
export function placeOrder(input: { cart: Cart; shipping: ShippingInput }): Order {
    ...
    // BUG B10: a flat $10.00 "handling fee" is added to the total.
    const order: Order = {
        id: `BZ-${orderSeq++}`,
        ...
        totalCents: totals.totalCents + 1000,
